...
public:
  ...
  void fill(const std::string& keyName,const boost::property_tree::ptree& psTree){
  ...
  void fill(const boost::property_tree::ptree& psTbl){
	...
	pathData.push_back(val);
      ...
      for(size_t indx=2;indx+1<pathData.size();indx++){
...
public:
  ...
  void fill(const boost::property_tree::ptree& runInfoTree,
    ...
    l1PSTbl_.fill(triggerMode_,l1PSTree);
    hltPSTbl_.fill(hltMenu_,hltPSTree);
  }
};
