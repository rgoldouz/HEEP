...
binsmll= array( 'd',[0,20,50,60,65,70,75,80,85,90,95,100,106,120,140,160,200,300,400,500,800,1200,2000] )
...
#regions=["ll","llOffZ"]
regions=["llB1InZ","llB1-BDTm1to0", "llB1-BDT0to0p8", "llB1-BDT0p8to1"]
regionsName=["ll","llOffZ","llB1", "llBg1"]
regionsName=["llB1InZ","llB1-BDTm1to0", "llB1-BDT0to0p8", "llB1-BDT0p8to1"]
...
for numyear, nameyear in enumerate(year):
    inp = ROOT.TFile.Open(HistAddress + nameyear+ '_ttbar.root' )
    tree = inp.Get("BNV")
    for numvar, namevar in enumerate(variables):
        canvas = ROOT.TCanvas(namevar,namevar,50,50,865,780)
        canvas.SetGrid();
        canvas.SetBottomMargin(0.17)
        canvas.cd()
        tree.Draw(namevar+":BDToutput","weight","prof")
        canvas.Print('ttbar_BDTCorr_'+namevar + ".png")
        del canvas
        gc.collect()

for numyear, nameyear in enumerate(year):
    inp = ROOT.TFile.Open(HistAddress + nameyear+ '_DY.root' )
    tree = inp.Get("BNV")
    for numvar, namevar in enumerate(variables):
        canvas = ROOT.TCanvas(namevar,namevar,50,50,865,780)
        canvas.SetGrid();
        canvas.SetBottomMargin(0.17)
        canvas.cd()
        tree.Draw(namevar+":lep1Pt")
        canvas.Print('DY_lep1PtCorr_'+namevar + ".png")
        del canvas
        gc.collect()
        canvas = ROOT.TCanvas(namevar,namevar,50,50,865,780)
        canvas.SetGrid();
        canvas.SetBottomMargin(0.17)
        canvas.cd()
        tree.Draw ( namevar +" >> h2 (30 ,0 ,1000) " )
        canvas.Print('DY_1D_'+namevar + ".png")
        del canvas
        gc.collect()
